SearchSeriesByCompanyAndYearComponent = {
    ext_lang: 'search_series_by_company_and_year_code',
    formats: ['format_search_series_by_company_and_year_json'],
    struct_support: true,

    factory: function (sandbox) {
        return new setSearchSeriesByCompanyAndYearViewerWindow(sandbox);
    }
};

var setSearchSeriesByCompanyAndYearViewerWindow = function (sandbox) {
    ...
    var keynodes = ['ui_search_series_by_company_and_year_in_memory'];

    $('#' + sandbox.container).prepend('<div class="inputBox" id="series-tools-' + sandbox.container + '"></div>');
    $('#series-tools-' + sandbox.container).load('static/components/html/search_series_by_company_and_year_component.html', function () {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonSearch = idf[keynodes['ui_search_series_by_company_and_year_in_memory']];
                ...
                $(buttonFind).click(function () {
                    ...
                    console.log("Series find " + yearString);
                    console.log("Series find company string" + cinocompanyString);
    ...
    this.applyTranslation = function (namesMap) {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonLoad = idf[keynodes['ui_search_series_by_company_and_year_in_memory']];
...
SCWeb.core.ComponentManager.appendComponentInitialize(SearchSeriesByCompanyAndYearComponent);
...
function findSeries(searchParams) {
    ...
    SCWeb.core.Server.resolveScAddr([searchParams.year, searchParams.cinocompany], function (keynodes) {
        ...
        SCWeb.core.Server.resolveScAddr(["ui_menu_file_for_finding_series_by_company_and_year"], function (data) {
            // Get command of ui_menu_view_full_semantic_neighborhood
            var cmd = data["ui_menu_file_for_finding_series_by_company_and_year"];
            console.log("cmd", cmd);
